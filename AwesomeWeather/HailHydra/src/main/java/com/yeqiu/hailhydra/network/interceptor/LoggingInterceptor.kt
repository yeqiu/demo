...
import com.yeqiu.hailhydra.network.NetLogger
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (builder.requestFlag) {
            if (request.method() == "GET") {
                NetLogger.printJsonRequest(builder, request)
            } else {
                if (subtypeIsNotFile(rSubtype)) {
                    NetLogger.printJsonRequest(builder, request)
                } else {
                    NetLogger.printFileRequest(builder, request)
                }
            }
        }
        ...
        if (builder.responseFlag) {
            ...
            if (subtypeIsNotFile(subtype)) {
                ...
                responseBody?.let {
                    ...
                    val bodyString = NetLogger.getJsonString(buffer.clone().readString(charset))
                    NetLogger.printJsonResponse(
                        builder,
                        chainMs,
                        isSuccessful,
                        code,
                ...
                NetLogger.printFileResponse(builder, chainMs, isSuccessful, code, header, segmentList)
            }
        }
